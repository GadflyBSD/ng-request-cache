	...
	.factory('unitFactory', function($q, $http, $window, $ionicLoading, $cordovaToast, app_config, toastr){
		...
		return {
			...
			loading: function(){
				...
				$ionicLoading.show({animation: 'fade-in', showBackdrop: true, maxWidth: 200, showDelay: 0, duration: app_config.duration_timeout});
			},
			...
			toastr: function(param, text, title) {
				var defer = $q.defer();
				if(angular.isObject(window.plugins.toast)){
					...
					$cordovaToast.showShortCenter(msg).then(function () {
						defer.resolve();
					});
				}else{
			...
			http: function(params, method){
				...
				if ($window.cordova && ionic.Platform.isIOS() && !app_config.debug.ios) {
					var header = {
