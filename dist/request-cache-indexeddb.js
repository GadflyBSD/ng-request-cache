/**
 * @license request-cache-indexeddb v1.0.0
...
angular.module('request-cache-indexeddb', [])
	...
	.factory('requestCacheFactory', function($q, $window, $cacheFactory, $indexedDB, $cordovaToast, $http, configs, unitFactory){
		return {
			/**
			 * 本地 IndexedDB 缓存服务器返回的数据
			 * @param data
			 * @param now
			 */
				...
				$http(config).success(function(result){
					if(result.type.toLowerCase() == 'success'){
						...
						if(!unitFactory.isEmptyObject(result.cache) && angular.isObject(result.cache)){
							_self.indexeddbStorage(result.cache, now);
						}
